...
const ItemDetalle = ({ item, allProducts }) => {
  ...
  useEffect(() => {
    if (allProducts && allProducts.length > 0) {
      const filtered = allProducts.filter(p => p.id !== item.id);
  ...
  useEffect(() => {
    setImagenSeleccionada(item.img);
  }, [item.id, item.img]);

  const allImages = [];
  if (item.img) allImages.push(item.img);
  if (item.imagenes) {
    if (Array.isArray(item.imagenes)) {
      item.imagenes.forEach(img => {
        if (!allImages.includes(img)) allImages.push(img);
      });
    } else if (typeof item.imagenes === 'string' && !allImages.includes(item.imagenes)) {
      allImages.push(item.imagenes);
    }
  }
  ...
  return (
    <>
      <div className="container-top-detail">
        <div className="top-and-bottom">
          <div className="top-menu">
            <span className="span-detail">También puede interesarte: </span>
            {suggestions.map((sug, index) => (
              <React.Fragment key={sug.id}>
                <NavLink to={`/detalle/${sug.id}`} style={{ textDecoration: 'none' }}>
                  <span className="span-detail" style={{ color: '#3483fa' }}>
                    {sug.nombre.length > 20 ? sug.nombre.slice(0, 20) + "..." : sug.nombre}
                  </span>
                </NavLink>
                {index < suggestions.length - 1 && <span className="span-detail"> - </span>}
              </React.Fragment>
            ))}
          </div>
      ...
      <div className='ui-pdp-container ui-pdp-container--pdp'>
        <div className='ui-pdp-container__row ui-pdp--relative ui-pdp-with--separator--fluid'>
          <div className='ui-pdp-container__col col-3 ui-pdp-container--column-center' style={{ zIndex: isZooming ? 2000 : 1 }}>
            <div className='ui-pdp--sticky-wrapper ui-pdp--sticky-wrapper-center' style={{ top: "-2015px", zIndex: isZooming ? 2000 : 1 }}>
              <div className='ui-pdp-container__row ui-pdp-with--separator--fluid ui-pdp-with--separator--40'>
                <div className='ui-pdp-container__col col-2 ui-pdp--relative'>
                  <div className='ui-pdp--sticky-wrapper ui-pdp--sticky-wrapper-gallery'>
                    <div className='ui-pdp-gallery'>
                      <div className={`ui-pdp-gallery__zoom-container ${isZooming ? 'show' : ''}`} style={{ display: isZooming ? 'block' : 'none' }}>
                        <div className='ui-pdp-gallery__zoom-zoom-pane'>
                          <img
                            ...
                            style={{
                              ...
                              height: "200%",
                              objectFit: "contain",
                              transform: `translate(-${zoomPosition.x / 2}%, -${zoomPosition.y / 2}%)`,
                              position: "absolute",
                              left: "0",
                              top: "0"
                            }}
                        ...
                        {allImages.map((img, index) => (
                          <React.Fragment key={index}>
                            <input
                              ...
                              value={index}
                              checked={imagenSeleccionada === img}
                              readOnly
                              onClick={() => setImagenSeleccionada(img)}
                            />
                            <span className='ui-pdp-gallery__wrapper'>
                              <label htmlFor={`gallery-thumbnail-${index}`} className='ui-pdp-gallery__label'>
                                <div className='ui-pdp-thumbnail ui-pdp-gallery__thumbnail'>
                                  <div className='ui-pdp-thumbnail__picture'>
                                    <img src={img} alt="" style={{ width: "44px", height: "44px" }} className="img-item" />
                                  </div>
                                </div>
                              </label>
                              {index === 0 && (
                                <figure
                                  className='ui-pdp-gallery__figure'
                                ...
                                >
                                  ...
                                  {isZooming && (
                                    <div
                                      className="ui-pdp-gallery__zoom-lens"
                                      style={{
                                        position: 'absolute',
                                        width: '150px',
                                        height: '150px',
                ...
                <div className='ui-pdp-container__col col-2' style={{ maxWidth: "352px" }}>
                  <div className='ui-pdp-container__top-wrapper'>
          ...
          <div className='ui-pdp-container__col col-1 ui-pdp-container--column-right mt-16 pr-16' style={{ alignItems: 'flex-start', display: 'flex', flexDirection: 'column' }}>
            <div className='ui-pdp--sticky-wrapper ui-pdp--sticky-wrapper-right' style={{ top: "-1063px" }}>
              <div className='ui-pdp-container__row'>
                <form action="" className='ui-pdp-buybox' id="buybox-form">
                  ...
                  <div className='ui-pdp-seller mb-20' style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                ...
                <div className='ui-box-component ui-box-component-pdp__visible--desktop'>
                  ...
                  <div className='ui-seller-info'>
                    ...
                    <div className='ui-seller-info__status-info'>
                      <div className='ui-seller-info__status-info__icon ui-pdp-color--GREEN'></div>
                      <div>
